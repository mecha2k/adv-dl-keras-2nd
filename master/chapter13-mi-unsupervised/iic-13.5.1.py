    ...
    def eval(self):
        ...
        for head in range(self.args.heads):
            ...
            accuracy = unsupervised_labels(
                list(self.y_test), list(y_head), self.n_labels, self.n_labels
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--save-dir", default="weights", help="Folder for storing model weights (h5)"
    )
    parser.add_argument(
        "--save-weights", default=None, help="Folder for storing model weights (h5)"
    )
    parser.add_argument("--dataset", default=mnist, help="Dataset to use")
    parser.add_argument(
        "--epochs", type=int, default=1200, metavar="N", help="Number of epochs to train"
    )
    parser.add_argument("--batch-size", type=int, default=512, metavar="N", help="Train batch size")
    parser.add_argument("--heads", type=int, default=1, metavar="N", help="Number of heads")
    ...
    parser.add_argument("--crop", type=int, default=4, help="Pixels to crop from the image")
    parser.add_argument(
