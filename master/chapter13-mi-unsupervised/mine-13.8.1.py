...
def sample(joint=True, mean=[0, 0], cov=[[1, 0.5], [0.5, 1]], n_data=1000000):
    ...
    """
    xy = np.random.multivariate_normal(mean=mean, cov=cov, size=n_data)
    # samples fr joint distribution
    if joint:
        return xy
    y = np.random.multivariate_normal(mean=mean, cov=cov, size=n_data)
...
class SimpleMINE:
    def __init__(self, args, input_dim=1, hidden_units=16, output_dim=1):
    ...
    def train(self):
        ...
        for epoch in range(self.args.epochs):
            ...
            loss_item = self._model.train_on_batch([x, y], np.zeros(x.shape))
            loss += loss_item
...
class LinearClassifier:
    ...
    def train(self, x_test, y_test):
        ...
        Arguments:
            ...
            y_test (tensor): Corresponding image label
                fr test dataset
        """
        self._model.fit(x_test, y_test, epochs=10, batch_size=128)
    ...
    def eval(self, x_test, y_test):
        ...
        score = self._model.evaluate(x_test, y_test, batch_size=128, verbose=0)
...
class MINE:
    ...
    def build_model(self):
        ...
        self._mine = SimpleMINE(
            self.args, input_dim=self.latent_dim, hidden_units=1024, output_dim=1
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--cov_xy", type=float, default=0.5, help="Gaussian off diagonal element")
    parser.add_argument("--save-dir", default="weights", help="Folder for storing model weights")
    parser.add_argument(
        "--save-weights", default=None, help="Filename (dim added) of model weights (h5)."
    )
    ...
    parser.add_argument(
        "--batch-size", type=int, default=1000, metavar="N", help="Train batch size"
    )
    parser.add_argument(
        "--gaussian", default=False, action="store_true", help="Compute MI of 2D Gaussian"
    )
    parser.add_argument(
        "--plot-model", default=False, action="store_true", help="Plot all network models"
    )
    parser.add_argument("--train", default=False, action="store_true", help="Train the model")
    parser.add_argument(
        "--latent-dim", type=int, default=10, metavar="N", help="MNIST encoder latent dim"
    )
    parser.add_argument("--restore-weights", default=None, help="Restore saved model weights")
    parser.add_argument(
        "--eval",
        default=False,
        action="store_true",
    ...
    if args.gaussian:
        ...
        if args.plot_model:
            plot_model(simple_mine.model, to_file="simple_mine.png", show_shapes=True)
    else:
        # build backbone
        ...
        if args.plot_model:
            plot_model(mine.classifier.model, to_file="classifier.png", show_shapes=True)
            plot_model(mine.encoder, to_file="encoder.png", show_shapes=True)
            plot_model(mine.model, to_file="model-mine.png", show_shapes=True)
        if args.train:
            mine.train()
