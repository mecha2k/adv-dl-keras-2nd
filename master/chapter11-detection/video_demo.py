...
class VideoDemo:
    def __init__(
        self, detector, camera=0, width=640, height=480, record=False, filename="demo.mp4"
    ):
        self.camera = camera
        ...
        while True:
            ...
            if count > 0:
                ...
                for key in items.keys():
                    ...
                    cv2.putText(image, display, (xmin + 10, ymin + 25), font, 0.55, (0, 0, 0), 1)
                    ymin += 30

                cv2.line(image, (xmin + 10, ymin), (xmax - 10, ymin), (0, 0, 0), 1)

                display = "P%0.2f Total" % (total)
                cv2.putText(image, display, (xmin + 5, ymin + 25), font, 0.75, (0, 0, 0), 1)
...
if __name__ == "__main__":
    ...
    help_ = "Record video"
    parser.add_argument("--record", default=False, action="store_true", help=help_)
    help_ = "Video filename"
    parser.add_argument("--filename", default="demo.mp4", help=help_)
    ...
    if args.restore_weights:
        ssd.restore_weights()
        videodemo = VideoDemo(
            detector=ssd, camera=args.camera, record=args.record, filename=args.filename
        )
        videodemo.loop()
