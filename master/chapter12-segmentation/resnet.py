...
def features_pyramid(x, n_layers):
    ...
    for i in range(n_layers - 1):
        ...
        conv = conv_layer(
            prev_conv, n_filters, kernel_size=3, strides=2, use_maxpool=False, postfix=postfix
        )
        outputs.append(conv)
        prev_conv = conv
...
def build_resnet(input_shape, n_layers=4, version=2, n=6):
...
if __name__ == "__main__":
    ...
    backbone = build_resnet(input_shape, n_layers=args.layers)
    backbone.summary()
