...
class FCN:
    ...
    def build_model(self):
        ...
        self.fcn = build_fcn(self.input_shape, self.backbone, self.n_classes)
    ...
    def segment_objects(self, image, normalized=True):
        ...
        segmentation = to_categorical(segmentation, num_classes=self.n_classes)
        if not normalized:
    ...
    def eval(self):
        ...
        for key in self.test_keys:
            ...
            if not self.args.train:
                log = "%s: %d objs, miou=%0.4f ,pla=%0.2f%%" % (key, n_masks, i_iou, i_pla)
                print_log(log, self.args.verbose)
        ...
        if m_iou > self.miou and self.args.train:
            ...
            print_log(log, self.args.verbose)
            self.miou = m_iou
            print_log("Saving weights... %s" % self.weights_path, self.args.verbose)
            self.fcn.save_weights(self.weights_path)
        else:
            log = "\nCurrent mIoU=%0.4f, Pixel level accuracy=%0.2f%%" % (m_iou, m_pla)
            print_log(log, self.args.verbose)
    ...
    def print_summary(self):
        ...
        if self.args.summary:
            ...
            self.fcn.summary()
            plot_model(self.fcn, to_file="fcn.png", show_shapes=True)
            plot_model(self.backbone, to_file="backbone.png", show_shapes=True)
